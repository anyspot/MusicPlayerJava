function OnStart()
{
    //Create a layout with objects vertically on Top.
    lay = app.CreateLayout( "linear", "VTop,FillXY" );    

    //Create a text label and add it to layout.
    txt = app.CreateText( "MediaAdPlayer" );
    txt.SetTextSize( 30 );
    lay.AddChild( txt );
    ...
    mp3List = app.ListFolder( "/storage/sdcard1/Adplayer/", ".mp3" ); 
    spin.SetList( mp3List ); 
     
    //Load the first file found. 
    player.SetFile( "/storage/sdcard1/Adplayer/" + spin.GetText() ); 
    dur = null; 
...
{ 
    player.SetFile( "/storage/sdcard1/Adplayer/" + item ); 
} 
...
{ 
    ...
    //Create a layout with objects vertically on Bottom.
    lay = app.CreateLayout( "linear", "VBottom,FillXY" );    

    //Create a text label and add it to layout.
    txt = app.CreateText( "Current Music" + item );
    txt.SetTextSize( 20 );
    lay.AddChild( txt );
